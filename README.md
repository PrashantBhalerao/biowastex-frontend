# ♻️ BioWastex - Smart Waste Segregation Frontend

BioWastex is a modern and responsive frontend web application designed to promote smart waste segregation awareness and eco-friendly waste management practices. The project focuses on educating users about proper waste classification through an interactive and visually engaging interface.

## 🚀 Features

- Responsive modern UI
- Eco-friendly themed design
- Waste category awareness sections
- Interactive frontend components
- Smooth navigation and layout
- Mobile-friendly design
- Clean and organized structure

## 🛠️ Technologies Used

- HTML5
- CSS3
- JavaScript
- Responsive Web Design

## 📌 Project Objective

The goal of BioWastex is to create a digital platform that spreads awareness about proper waste segregation and sustainable waste management practices. The project demonstrates modern frontend development concepts while solving a real-world environmental problem.

## 🌱 Future Enhancements

The project will later be upgraded into a complete full-stack application with:

- User Authentication
- Database Integration
- Admin Dashboard
- AI-based Waste Classification
- Waste Submission Tracking
- Analytics Dashboard
- React Frontend
- Django Backend
- MySQL Database

## 📷 Screenshots

<img width="1877" height="851" alt="Screenshot 2026-05-27 172104" src="https://github.com/user-attachments/assets/63ca3dfa-3838-4284-afdb-86d3ca240e4c" />
<img width="1891" height="852" alt="Screenshot 2026-05-27 172233" src="https://github.com/user-attachments/assets/b809ab27-0bd1-420d-8ecc-57d1752c9166" />
<img width="1893" height="851" alt="Screenshot 2026-05-27 172220" src="https://github.com/user-attachments/assets/5c1d206d-33a4-42d3-b803-6dccd2d00868" />
<img width="1877" height="852" alt="Screenshot 2026-05-27 172207" src="https://github.com/user-attachments/assets/4f02dbd8-7ddf-4be3-9448-d4b3d5795082" />
<img width="1870" height="842" alt="Screenshot 2026-05-27 172153" src="https://github.com/user-attachments/assets/9709ca20-2712-4fae-a0d5-017d26cadd19" />
<img width="1877" height="851" alt="Screenshot 2026-05-27 172104" src="https://github.com/user-attachments/assets/9a399c4f-5e87-4edc-a8c0-c1444a50f014" />
<img width="1880" height="845" alt="Screenshot 2026-05-27 172132" src="https://github.com/user-attachments/assets/0decc400-190d-450f-9a57-8d52f8fbf28a" />






## 🔗 Live Demo

[_Add deployed project link here._](https://biowastex-byprashant.netlify.app/)

## 👨‍💻 Author

Prashant Kishor Bhalerao

- GitHub: https://github.com/PrashantBhalerao
- LinkedIn: www.linkedin.com/in/prashant-bhalerao

## 📄 License

This project is developed for educational and portfolio purposes.
